...
class Approx_Huber_Loss(nn.Module):
    def __init__(self):
        ...
        self.sobel_filter_X = torch.from_numpy(self.sobel_filter_X).float().unsqueeze(0).unsqueeze(0)
        self.sobel_filter_Y = torch.from_numpy(self.sobel_filter_Y).float().unsqueeze(0).unsqueeze(0)
